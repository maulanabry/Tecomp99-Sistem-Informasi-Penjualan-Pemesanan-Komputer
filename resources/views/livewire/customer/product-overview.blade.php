        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 lg:py-12">
            <div class="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-12">
                ...
                <div class="lg:col-span-7 space-y-6">
                    ...
                    <!-- Image Gallery -->
<div class="bg-white/80 backdrop-blur-sm rounded-3xl overflow-hidden border border-gray-200/60 transition-all duration-700 hover:scale-[1.01] hover:border-primary-300/50 relative group">
                    @if($product->images->count() > 0)
                        ...
                        <div class="aspect-square bg-gray-50/50 relative overflow-hidden cursor-zoom-in" onclick="openImageModal('{{ asset($currentImage->url ?? $product->images->first()->url) }}', '{{ $product->name }}')">
                            ...
                            @if($product->images->count() > 1)
                                <button wire:click.stop="selectImage({{ $selectedImageIndex > 0 ? $selectedImageIndex - 1 : $product->images->count() - 1 }})"
                                ...
                                <button wire:click.stop="selectImage({{ $selectedImageIndex < $product->images->count() - 1 ? $selectedImageIndex + 1 : 0 }})"
                ...
                <div class="lg:col-span-5 space-y-6">
                    ...
                    <!-- Product Info Card -->
<div class="bg-white/90 backdrop-blur-sm rounded-3xl p-6 lg:p-8 transition-all duration-700 border border-gray-200/60 hover:border-primary-300/50 hover:scale-[1.01] sticky top-6">
                    
                        <!-- Category Badge -->
                        <div class="mb-4">
                            <span class="inline-flex items-center px-4 py-2 rounded-full text-sm font-semibold bg-gradient-to-r from-primary-50 to-primary-100 text-primary-800 border border-primary-200/50 hover:from-primary-100 hover:to-primary-200 transition-all duration-300 shadow-sm">
                        ...
                        <div class="mb-8 p-4 bg-gradient-to-r from-primary-50 to-orange-50 rounded-2xl border border-primary-100/50">
                            <div class="flex items-baseline space-x-2">
                                <span class="text-3xl lg:text-4xl font-bold bg-gradient-to-r from-primary-600 to-orange-600 bg-clip-text text-transparent">
                                    Rp {{ number_format($product->price, 0, ',', '.') }}
                                </span>
                                <span class="text-sm text-gray-500 font-medium">per unit</span>
                            </div>
                        </div>
                    
                        <!-- Stock &amp; Sold Count with modern cards -->
<div class="grid grid-cols-2 gap-4 mb-8">
    <div class="bg-gradient-to-br from-white to-gray-50 rounded-2xl p-4 border border-gray-200/50">
                                @if($product->stock > 0)
                                    <div class="flex items-center space-x-3">
                                        ...
                                        <div>
                                            ...
                                            <div class="text-sm font-bold text-gray-900">{{ number_format($product->stock) }} tersedia</div>
                                        </div>
                                    </div>
                                @else
                                    <div class="flex items-center space-x-3">
                                        ...
                                        <div>
                                            <div class="text-xs text-gray-500 font-medium uppercase tracking-wide">Stok</div>
                                            <div class="text-sm font-bold text-red-600">Habis</div>
                                        </div>
                                    </div>
                                @endif
                            </div>
<div class="bg-gradient-to-br from-white to-gray-50 rounded-2xl p-4 border border-gray-200/50">
                                <div class="flex items-center space-x-3">
                                    ...
                                    <div>
                                        ...
                                        <div class="text-sm font-bold text-gray-900">{{ number_format($product->sold_count) }}</div>
                                    </div>
                                </div>
                            </div>
                        </div>
...
<button wire:click="toggleWishlist" 
        class="inline-flex items-center space-x-3 px-6 py-3 rounded-2xl text-sm font-semibold transition-all duration-300 hover:scale-105 {{ in_array($product->product_id, $wishlist) ? 'bg-gradient-to-r from-red-50 to-pink-50 text-red-700 hover:from-red-100 hover:to-pink-100 border border-red-200/50' : 'bg-gradient-to-r from-gray-50 to-gray-100 text-gray-700 hover:from-gray-100 hover:to-gray-200 border border-gray-200/50' }}">
                            ...
                            @if($product->stock > 0)
                                <div class="space-y-3">
                                    <label class="text-sm font-semibold text-gray-700 uppercase tracking-wide">Jumlah</label>
                                    <div class="flex items-center space-x-4">
                                        <div class="flex items-center bg-gradient-to-r from-white to-gray-50 border border-gray-200/60 rounded-2xl shadow-sm hover:shadow-md transition-all duration-300">
                                            <button wire:click="decrementQuantity" 
                                                    class="px-4 py-3 text-gray-600 hover:text-primary-600 hover:bg-primary-50 transition-all duration-200 rounded-l-2xl group"
                                                    {{ $quantity <= 1 ? 'disabled' : '' }}>
                                            ...
                                            <input type="number" 
                                                   ...
                                                   class="w-16 px-3 py-3 text-center border-0 focus:ring-0 focus:outline-none bg-transparent font-bold text-gray-900">
                                            <button wire:click="incrementQuantity" 
                                                    class="px-4 py-3 text-gray-600 hover:text-primary-600 hover:bg-primary-50 transition-all duration-200 rounded-r-2xl group"
...
<button wire:click="addToCart" 
        class="w-full bg-gradient-to-r from-gray-100 to-gray-200 hover:from-gray-200 hover:to-gray-300 text-gray-800 font-bold py-4 px-8 rounded-2xl transition-all duration-300 hover:scale-105 flex items-center justify-center space-x-3 border border-gray-300/50 group">
                                    <i class="fas fa-shopping-cart w-5 h-5 group-hover:scale-110 transition-transform"></i>
                                    <span>Tambah ke Keranjang</span>
                                </button>
                                
<button wire:click="buyNow" 
        class="w-full bg-gradient-to-r from-primary-500 to-orange-500 hover:from-primary-600 hover:to-orange-600 text-white font-bold py-4 px-8 rounded-2xl transition-all duration-300 hover:scale-105 flex items-center justify-center space-x-3 group">
                            ...
                            @else
                                <button disabled 
                                        class="w-full bg-gradient-to-r from-gray-200 to-gray-300 text-gray-500 font-bold py-4 px-8 rounded-2xl cursor-not-allowed flex items-center justify-center space-x-3 opacity-60">
...
<div class="bg-white/90 backdrop-blur-sm rounded-3xl border border-gray-200/60 transition-all duration-700 overflow-hidden hover:border-primary-300/50">
                    <div class="p-8 lg:p-12">
                        <!-- Section Header -->
                        <div class="flex items-center space-x-4 mb-8">
                            <div class="w-1 h-8 bg-gradient-to-b from-primary-500 to-orange-500 rounded-full"></div>
                            <h2 class="text-2xl lg:text-3xl font-bold text-gray-900">Deskripsi Produk</h2>
                        </div>
                ...
                <div class="bg-white/90 backdrop-blur-sm rounded-3xl border border-gray-200/60 transition-all duration-700 overflow-hidden hover:border-primary-300/50">
                    <div class="p-8 lg:p-12">
                        ...
                        <div class="flex items-center space-x-4 mb-8">
                            ...
                            <h2 class="text-2xl lg:text-3xl font-bold text-gray-900">Produk Terkait</h2>
                        </div>
