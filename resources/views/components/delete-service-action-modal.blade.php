    ...
    <div class="relative w-full max-w-md max-h-full">
        ...
        <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
            ...
            <div class="flex items-center justify-center p-6 space-x-2 border-t border-gray-200 rounded-b dark:border-gray-600">
                <form action="@if(auth('teknisi')->check()){{ route('teknisi.service-tickets.actions.destroy', [$action->serviceTicket, $action]) }}@else{{ route('service-tickets.actions.destroy', [$action->serviceTicket, $action]) }}@endif" method="POST" class="inline">
                    @csrf
                    @method('DELETE')
