    ...
    <div class="py-6">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 md:px-8">
            <div class="flex items-center justify-between">
                <h1 class="text-2xl font-semibold text-gray-900 dark:text-gray-100">Tambah Kategori</h1>
            </div>
        </div>
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 md:px-8">
            <div class="py-4">
                <div class="bg-white dark:bg-gray-800 rounded-lg shadow overflow-hidden">
                    <form action="{{ route('categories.store') }}" wire:navigate method="POST" class="p-6 space-y-6">
                        @csrf
                        ...
                        <div>
                            ...
                            <select name="type" id="type" required
                                ...
                                <option value="layanan" {{ old('type') === 'layanan' ? 'selected' : '' }}>Layanan</option>
                            </select>
                            @error('type')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
