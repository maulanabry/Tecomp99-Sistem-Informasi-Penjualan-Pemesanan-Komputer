<x-layout-admin>
                    ...
                    <form action="{{ route('products.update', $product->product_id) }}" method="POST" enctype="multipart/form-data" class="space-y-6 p-6">
                        ...
                        <div>
                            <label class="block text-sm font-medium text-gray-700 dark:text-gray-200 mb-2">
                                Gambar Saat Ini <span id="currentImageCounter" class="text-sm text-gray-500">({{ $product->images->count() }}/6 Images)</span>
                            </label>
                            <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
                                @foreach($product->images as $image)
                                    <div class="relative" id="image-{{ $image->id }}">
                                        <div class="relative w-40 h-40 bg-gray-100 dark:bg-gray-800 border-2 border-gray-300 dark:border-gray-600 rounded-lg flex items-center justify-center {{ $image->is_main ? 'ring-2 ring-primary-500' : '' }}">
                                            <img src="{{ asset($image->url) }}" alt="{{ $product->name }}" class="object-contain w-32 h-32">
                                            <div class="absolute top-0 right-0 flex space-x-1">
                                                @if(!$image->is_main)
                                                    <button type="button" onclick="setMainImage('{{ $product->product_id }}', {{ $image->id }})"
                                                        class="p-1.5 bg-blue-500 text-white rounded-lg focus:ring-2 focus:ring-blue-300 hover:bg-blue-600 inline-flex items-center justify-center h-8 w-8">
                                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"></path>
                                                        </svg>
                                                    </button>
                                                @endif
                                                <button type="button" onclick="deleteImage('{{ $product->product_id }}', {{ $image->id }})"
                                                    class="p-1.5 bg-white text-gray-400 hover:text-gray-900 rounded-lg focus:ring-2 focus:ring-gray-300 hover:bg-gray-100 inline-flex items-center justify-center h-8 w-8 dark:text-gray-500 dark:hover:text-white dark:bg-gray-800 dark:hover:bg-gray-700">
                            ...
                            <div class="mt-1">
                                <label for="dropzone-file" class="flex flex-col items-center justify-center w-full h-64 border-2 border-gray-300 border-dashed rounded-lg cursor-pointer bg-gray-50 dark:hover:bg-gray-800 dark:bg-gray-700 hover:bg-gray-100 dark:border-gray-600 dark:hover:border-gray-500 dark:hover:bg-gray-600">
                                    <div class="flex flex-col items-center justify-center pt-5 pb-6">
                                        ...
                                        <p class="text-xs text-gray-500 dark:text-gray-400">PNG, JPG, GIF up to 2MB (Max 6 images total)</p>
                                    </div>
    ...
    <script>
        ...
        function handleImageUpload() {
            ...
            const currentImages = document.querySelectorAll('.image-preview-item, #image-\\d+').length;
        ...
        function updateImageCounter() {
            const currentCount = document.querySelectorAll('.image-preview-item, #image-\\d+').length;
            currentImageCounter.textContent = `(${currentCount}/6 Images)`;
        }
        ...
        function deleteImage(productId, imageId) {
            ...
            .then(data => {
                if (data.success) {
                    ...
                    alert(data.message || 'Image successfully deleted');
                }
            })
            .catch(error => {
        ...
        function setMainImage(productId, imageId) {
            ...
            .then(data => {
                if (data.success) {
                    ...
                    document.querySelectorAll('.relative.w-40.h-40').forEach(container => {
                        container.classList.remove('ring-2', 'ring-primary-500');
                    });
                    // Add ring to new main image
                    document.querySelector(`#image-${imageId} .relative.w-40.h-40`).classList.add('ring-2', 'ring-primary-500');
                    // Show success message
                    alert(data.message || 'Main image updated successfully');
                }
            })
            .catch(error => {
                console.error('Error:', error);
                alert(error.message || 'Failed to set main image. Please try again.');
            });
        }
