<x-layout-customer>
    <x-slot name="title">Invoice Pesanan Produk - {{ $order->order_product_id }}</x-slot>
    <x-slot name="description">Invoice pesanan produk Anda di Tecomp99.</x-slot>
        ...
        <script>
            function downloadPDF() {
                ...
                printWindow.document.write(`
                    ...
                    <html>
                    <head>
                        <title>Invoice #{{ $order->order_product_id }} - {{ $order->customer->name }}</title>
                        <meta charset="UTF-8">
        ...
        <div id="invoice-content" class="max-w-4xl mx-auto px-4 sm:px-6 md:px-8">
            <div class="bg-white p-4 sm:p-6 rounded-lg shadow-sm border border-gray-100">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6 mb-6 p-4 bg-gray-50 rounded-lg">
                    <div>
                        <h2 class="text-lg font-bold text-gray-900 mb-2">Invoice #{{ $order->order_product_id }}</h2>
                        <div>
                            ...
                            <span class="ml-2 inline-flex px-2 py-1 rounded-full text-xs font-medium
                                {{ $order->type === 'pengiriman' ? 'bg-blue-100 text-blue-800' : 'bg-purple-100 text-purple-800' }}">
                                {{ ucfirst($order->type) }}
                            </span>
                        </div>
                    </div>
                    <div class="text-left sm:text-right">
                        ...
                        <p class="text-base font-semibold text-gray-900">{{ $order->created_at->format('d/m/Y') }}</p>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-6">
                    <div class="p-3 bg-gray-50 rounded">
                        ...
                        <div class="text-xs space-y-1">
                            ...
                            <p><span class="font-medium">Alamat:</span> {{ $order->customer->addresses->first()?->detail_address ?? 'Alamat tidak tersedia' }}</p>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 print-grid">
                    <div class="flex flex-col justify-start space-y-4">
                        ...
                        <div>
                            ...
                            <div class="space-y-2">
                                <p class="inline-flex px-2 py-1 rounded-full text-xs
                                    {{ $order->status_payment === 'belum_dibayar' ? 'bg-red-100 text-red-800' : '' }}
                                    {{ $order->status_payment === 'down_payment' ? 'bg-yellow-100 text-yellow-800' : '' }}
                                    {{ $order->status_payment === 'lunas' ? 'bg-green-100 text-green-800' : '' }}
                                    {{ $order->status_payment === 'dibatalkan' ? 'bg-gray-100 text-gray-800' : '' }}">
                                    {{ str_replace('_', ' ', ucfirst($order->status_payment)) }}
                                </p>
                                <div class="text-xs space-y-1">
                                    <p><span class="font-medium">Sudah Dibayar:</span> Rp {{ number_format($order->paid_amount, 0, ',', '.') }}</p>
                                    @if($order->remaining_balance > 0)
                                        <p class="text-red-600"><span class="font-medium">Sisa Pembayaran:</span> Rp {{ number_format($order->remaining_balance, 0, ',', '.') }}</p>
                                    @endif
                                    @if($order->last_payment_at)
                                        <p class="text-gray-500"><span class="font-medium">Pembayaran Terakhir:</span> {{ \Carbon\Carbon::parse($order->last_payment_at)->format('d/m/Y H:i') }}</p>
                                    @endif
                                </div>
                            </div>
                        </div>
                        ...
                        <div>
                            ...
                            <div class="space-y-2">
                                <p class="text-xs"><span class="font-medium">Masa Garansi:</span> {{ $order->warranty_period_months }} Bulan</p>
                                @if($order->warranty_expired_at)
                                    ...
                                    <p class="text-xs text-gray-500">
                                        <span class="font-medium">Berlaku sampai:</span> {{ \Carbon\Carbon::parse($order->warranty_expired_at)->format('d/m/Y') }}
                                    </p>
                                @endif
                            </div>
                        </div>
                    ...
                    <div class="border-t md:border-t-0 pt-2 md:pt-0 print:border-t-0 print:pt-0">
                        <div class="w-full max-w-sm ml-auto">
                            ...
                            <div class="grid grid-cols-2 gap-2">
                                ...
                                <div class="text-xs text-right">Rp {{ number_format($order->sub_total, 0, ',', '.') }}</div>
                                
                                @if($order->discount_amount > 0)
                                <div class="text-xs text-gray-600">Diskon:</div>
                                <div class="text-xs text-right">Rp {{ number_format($order->discount_amount, 0, ',', '.') }}</div>
                                @endif
                                
                                @if($order->shipping_cost > 0)
                                <div class="text-xs text-gray-600">Ongkir:</div>
                                <div class="text-xs text-right">Rp {{ number_format($order->shipping_cost, 0, ',', '.') }}</div>
                                @endif
                                
                                <div class="text-sm font-semibold">Grand Total:</div>
                                <div class="text-sm font-semibold text-right">Rp {{ number_format($order->grand_total, 0, ',', '.') }}</div>
                            </div>
                        </div>
                    </div>
                </div>
