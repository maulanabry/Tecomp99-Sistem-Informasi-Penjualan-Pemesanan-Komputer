<x-layout-customer>
    <x-slot name="title">Invoice Pesanan Servis - {{ $order->order_service_id }}</x-slot>
    <x-slot name="description">Invoice pesanan layanan servis Anda di Tecomp99.</x-slot>
        ...
        <script>
            function downloadPDF() {
                ...
                printWindow.document.write(`
                    ...
                    <html>
                    <head>
                        <title>Invoice Servis #{{ $order->order_service_id }} - {{ $order->customer->name }}</title>
                        <meta charset="UTF-8">
        ...
        <div id="invoice-content" class="max-w-4xl mx-auto px-4 sm:px-6 md:px-8">
            <div class="bg-white p-4 sm:p-6 rounded-lg shadow-sm border border-gray-100">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6 mb-6 p-4 bg-gray-50 rounded-lg">
                    <div>
                        <h2 class="text-lg font-bold text-gray-900 mb-2">Invoice #{{ $order->order_service_id }}</h2>
                        <div>
                            ...
                            <span class="ml-2 inline-flex px-2 py-1 rounded-full text-xs font-medium
                                {{ $order->type === 'reguler' ? 'bg-blue-100 text-blue-800' : 'bg-purple-100 text-purple-800' }}">
                                {{ ucfirst($order->type) }}
                            </span>
                        </div>
                    </div>
