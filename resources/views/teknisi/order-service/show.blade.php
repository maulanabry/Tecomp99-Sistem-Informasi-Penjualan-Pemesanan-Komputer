<x-layout-teknisi>
    <div class="max-w-7xl mx-auto p-6">
        ...
        <div class="mb-6">
            ...
            <div class="flex flex-wrap items-center gap-2 mb-6">
                ...
                <!-- Create Service Ticket Button (if conditions are met) -->
                ...
                @if($orderService->paymentDetails->isEmpty())
                    {{-- <a href="{{ route('teknisi.payments.create') }}?order_service_id={{ $orderService->id }}"
                ...
                <div>
                    <dl class="space-y-3">
                        <div>
                            ...
                            <dd class="text-sm text-gray-900 dark:text-white">{{ $orderService->customer->name }}</dd>
                        </div>
                        <div>
                            <dt class="text-sm font-medium text-gray-500 dark:text-gray-400">Email</dt>
                            <dd class="text-sm text-gray-900 dark:text-white">{{ $orderService->customer->email }}</dd>
                        </div>
                        <div>
                            <dt class="text-sm font-medium text-gray-500 dark:text-gray-400">Nomor Telepon</dt>
                            <dd class="text-sm text-gray-900 dark:text-white">{{ $orderService->customer->contact }}</dd>
                        </div>
                    </dl>
                </div>
                <div>
                    ...
                    @foreach($orderService->tickets as $ticket)
                        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-6">
                            ...
                            <dd class="mt-1 text-sm text-gray-900 dark:text-gray-100">
                                <a href="{{ route('teknisi.service-tickets.show', $ticket) }}" class="text-primary-600 hover:underline">
                ...
                @if($orderService->paymentDetails->isNotEmpty())
                    @foreach($orderService->paymentDetails as $payment)
                        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-6">
                            ...
                            <dd class="mt-1 text-sm text-gray-900 dark:text-gray-100">
                                <a href="{{ route('teknisi.payments.show', $payment) }}" class="text-primary-600 hover:underline">
                                    {{ $payment->payment_id ?? '-' }}
                                </a>
                            </dd>
                            </div>
    ...
    {{-- <x-cancel-order-modal
        ...
        :action="route('teknisi.order-services.cancel', $orderService)"
    /> --}}
