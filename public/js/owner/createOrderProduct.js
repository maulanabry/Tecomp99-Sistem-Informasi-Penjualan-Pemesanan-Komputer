    ...
    async handleApplyPromo() {
        try {
            ...
            const response = await $.ajax({
                url: "/pemilik/order-produk/validate-voucher",
                method: "POST",
                headers: {
        ...
        if (!response.ok) {
            throw new Error(
                response_data.message ||
                    `Gagal mencari destinasi: ${response.status}`
            );
        }

        // Validate response data structure - handle the new API response format
...
async function calculateShippingCost(destination, weight) {
    try {
        ...
        // Handle the API response format - check if it has data wrapper
        const data = response_data.data || response_data;
