...
class ServiceTicketOrderSelectionModal extends Component
{
    ...
    {
        \Log::info('Teknisi ServiceTicketOrderSelectionModal: Opening modal');
        $this->show = true;
        $this->reset(['searchQuery', 'typeFilter', 'selectedOrder']);
        $this->resetPage();
        \Log::info('Teknisi ServiceTicketOrderSelectionModal: Modal opened successfully');
    }
    ...
    {
        \Log::info('Teknisi ServiceTicketOrderSelectionModal: Selecting order', ['orderId' => $orderId]);
        ...
        if ($order) {
            ...
            \Log::info('Teknisi ServiceTicketOrderSelectionModal: Dispatching order data', $orderData);

            // Kirim event ke parent component dengan data order sebagai object
            $this->dispatch('serviceTicketOrderSelected', $orderData);
        } else {
            \Log::error('Teknisi ServiceTicketOrderSelectionModal: Order not found', ['orderId' => $orderId]);
        }
    ...
    {
        return view('livewire.teknisi.service-ticket-order-selection-modal', [
            'orders' => $this->orders,
        ]);
    }
}
